<div class="projetos-igreja">
    <br><br>
    <h3>Projeto Educacional</h3>

    <a id="imagem-logo-adad">
        <img src="/assets/img/igreja/adad_logo.png" alt="logo do projeto ADAD">
    </a>

    <ul class="projeto-lista" id="projeto-lista-adad">
        <p>
            Desenvolvem atividades com crianças, adolescentes e jovens visando fortalecer o Caráter de Cristo, alores, princípios morais e éticos, patriotismo, recreação, acampamentos, prática de escotismo e integração em busca de uma formação educacional por princípios. 
        </p>
        <br>
        <li>
            Realização de Palestras educativas de socialização para todas as faixas etárias.
        </li>
    </ul>
</div>